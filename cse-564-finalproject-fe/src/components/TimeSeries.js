...
export default function TimeSeries({
  ...
  // Draw hourly chart AND attach brush
  useEffect(() => {
    if (hourlyLoading || !hourlyData.length || !dimensions.width) return;

    const svg = d3
      .select(hourlyChartRef.current)
    ...
    const gradientId = `hourly-gradient-${Math.random().toString(36).slice(2)}`;
    const gradient = svg
      .append("defs")
    ...
    g.selectAll(".point")
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip
          .html(
            `<div style="font-weight:bold">${d.hour}:00</div>${d.count} accidents`
          )
    ...
    const brush = d3
      ...
      .on("end", (event) => {
        if (!event.selection) {
          if (timeRange != null) onTimeRangeSelect(null);
          return;
        }
        const [p0, p1] = event.selection;
        const newStart = Math.floor(xBrush.invert(p0));
        const newEnd = Math.ceil(xBrush.invert(p1));
        // if it’s exactly the same, bail out:
        if (timeRange?.start === newStart && timeRange?.end === newEnd) return;
        onTimeRangeSelect({ start: newStart, end: newEnd });
      });

    const brushG = svg.append("g").attr("class", "brush").call(brush);

    // re-hydrate existing selection
    if (timeRange) {
      const x0 = xBrush(timeRange.start);
      const x1 = xBrush(timeRange.end);
  ...
  // Render day-of-week heatmap (unchanged)
  useEffect(() => {
    if (weekdayLoading || !weekdayData.length || !dimensions.width) return;
    const svg = d3
      .select(weekdayChartRef.current)
      .attr("viewBox", `0 0 ${dimensions.width} ${dimensions.height / 2}`)
      .attr("preserveAspectRatio", "xMidYMid meet");
    ...
    g.selectAll(".cell")
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip
          .html(
            `<div style="font-weight:bold">${d.day}, ${
              d.hour
            }:00</div>Accidents: ${Math.round(d.value * 100)}%`
          )
          .style("left", `${event.pageX + 10}px`)
          .style("top", `${event.pageY - 30}px`)
          .transition()
    ...
    g.append("g")
      .attr("transform", `translate(0,${innerH})`)
      .call(d3.axisBottom(xScale).tickFormat((h) => `${h}`))
      .selectAll("text")
      .attr("font-size", "9px")
      .attr("font-family", "system-ui, sans-serif")
      .attr("fill", theme.text)
